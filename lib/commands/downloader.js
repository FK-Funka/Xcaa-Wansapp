...
(async () => {
     switch(true) {
         ...
         case /^asupan$/i.test(command): {
             ...
             vidnya = await conn.prepareMessage(from, asupan, 'videoMessage')
             const butt = [
                 {buttonId:`${prefix + command}`,buttonText:{displayText:'Next'},type:1}
             ]
             const buttonMess = {
             videoMessage: vidnya.message.videoMessage,
             contentText: `Video Asupannya Nih Kak`,
             footerText: '© FxSx',
             buttons: butt,
             headerType: 5
             }
             await conn.sendMessage(from, buttonMess, 'buttonsMessage', {quoted: m, mimetype: 'video/mp4', contextInfo: {mentionedJid: [sender]}})
             break
          }
     }
})()