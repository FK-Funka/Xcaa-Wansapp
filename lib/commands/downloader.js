...
(async () => {
     switch(true) {
         ...
         case /^(pindl)$/i.test(command): {
             if (!args[0]) return reply('Url Pinterestnya?')
             await reply(mess.wait)
             let api = await functions.fetchJson(API('zaki', '/pindl', { url: args[0] }, 'apikey'))
             xcaa.sendFile(from, api.result.result, '', api.result.result, m)
             break
         }
         case /^yt(v|mp4)?$/i.test(command): {
             if (!args[0]) return reply('Url YouTubenya?')
             await reply(mess.wait)
             ...
             xcaa.sendFile(from, url, '', api.result.title, m)
             break
         }
         case /^yt(a|mp3)$/i.test(command): {
             ...
             let audio = await getBuffer(url)
             xcaa.sendMessage(from, audio, MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted: m })
             break
         }
         case /^(tik(tok)?(dl)?)$/i.test(command): {
             if (!args[0]) return reply('Url Tiktoknya?')
             await reply(mess.wait)
             xcaa.sendFile(from, API('zaki', '/tiktoknowm', { url: args[0] }, 'apikey'), '', '', m)
             break
         }
         case /^mediafire$/i.test(command): {
             if (!args[0]) return reply('Url MediaFirenya?')
             ...
             xcaa.sendFile(from, link, title, '', m)
             break
         }
         case /^z(ippydl|ippyshare)$/i.test(command): {
             if (!args[0]) return reply('Url Zippysharenya?')
             let api = await functions.fetchJson(API('zaki', '/zippyshare', { url: args[0] }, 'apikey'))
             let { nama, link } = api.result
             reply(JSON.stringify(api.result, null, 2))
             xcaa.sendFile(from, link, nama, '', m)
             break
         }
         case /^(twt|twitter)$/i.test(command): {
             if (!args.length > 0 || !args[0].includes('twitter.com') || args[0].includes('t.co')) return reply('Url Twitternya?')
         ...
         case /^(fbdl)$/i.test(command): {
             ...
             if (api.result.length === 0) return reply('This link appears to be leading to a private post.')
                 xcaa.sendFile(from, api.result[api.result.length - 1], '', '', m)
             } catch(e) {
         ...
         case /^anonfiles$/i.test(command): {
             if (!args[0]) return reply('Url Anonfilenya?')
         ...
         case /^sfilemobidl$/i.test(command): {
             if (!args[0]) return reply('Url Sfilemobinya?')
         ...
         case /^pixiv$/i.test(command): {
             if (args[0] && !isUrl(args[0])) {
                 await reply(mess.wait)
                 xcaa.sendFile(from, API('zaki', '/pixiv', { id: args[0], ext: args[1] || '.jpg' }, 'apikey'), '', '', m)
             } else if (args[0] && isUrl(args[0])) {
                 await reply(mess.wait)
                 xcaa.sendFile(from, API('zaki', '/pixiv', { id: args[0].split('/')[5], ext: args[1] || '.jpg' }, 'apikey'), '', '', m)
             } else reply(`*For Single Image*:\n${prefix + command} 86668346\n*For Multiple Images:*\n\n${prefix + command} 88314128-1\n(-1 is page number)`)
             break
         }
         case /^asupan$/i.test(command): {
             reply(mess.wait)
             sendButtVideo(from, `Video Asupannya Nih Kak`, '© FxSx', {jpegThumbnail: global.thumb}, 
             [
             {buttonId:`${prefix}asupan`,buttonText:{displayText:'NEXT'},type:1}
             ], 
             {mimetype: 'video/mp4', contextInfo: {mentionedJid: [sender]} quoted: m})
             break
          }
     }
})()