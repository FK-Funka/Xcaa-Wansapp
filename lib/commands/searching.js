...
(async () => {
     switch(true) {
         case /^(stic?kerline)$/i.test(command): {
             ...
             xcaa.sendFile(from, api[0].sticker, '', txt, m)
             break
         }
         ...
         case /^gimage$/i.test(command): {
             ...
             xcaa.sendFile(from, url, '', '*Google Image*: ' + text, m)
             break
         }
         case /^(pinterest)$/i.test(command): {
             ...
             xcaa.sendFile(from, url, '', '*Hasil pencarian dari*: ' + text, m)
             break
         }
         case /^(konachan)$/i.test(command): {
             if (!text) return reply('Query needed.')
             await reply(mess.wait)
             xcaa.sendFile(from, API('zaki', '/konachan', { query: text }, 'apikey'), '', '', m)
             break
         }
         ...
         case /^(happymod)$/i.test(command): {
             ...
             xcaa.sendFile(from, api.result[0].icon, '', txt, m)
             break
         }
         ...
         case /^(alphacoders)$/i.test(command): {
             if (!text) return reply('Query needed.')
             await reply(mess.wait)
             xcaa.sendFile(from, API('zaki', '/alphacoders', { query: text }, 'apikey'), '', '', m)
             break
         }
         case /^(wallpaper|wp)cave$/i.test(command): {
             if (!text) return reply('Query needed.')
             await reply(mess.wait)
             xcaa.sendFile(from, API('zaki', '/wallpapercave', { query: text }, 'apikey'), '', '', m)
             break
         }
         case /^(stelesearch)$/i.test(command): {
             ...
             xcaa.sendFile(from, api[0].icon, '', txt, m)
             break
         }
