     ...
     switch(true) {
        ...
        case /^to(mp3|a(udio)?)$/i.test(command): {
            ...
            if (isQuotedDocument && /video/.test(quoted.mimetype) || /video/.test(media.mtype)) {
            	reply(mess.wait)
                await convert.toAudio(await media.download(), 'mp4').then(v => conn.sendMessage(from, v, 'audioMessage', {quoted: ftroli, mimetype: 'audio/mp4' }))
            } else if (isQuotedDocument && /audio/.test(quoted.mimetype) || isQuotedAudio) {
            	reply(mess.wait)
                await convert.toPTT(await media.download(), 'mp4').then(v => conn.sendMessage(from, v, 'audioMessage', {quoted: ftroli, ptt: true, mimetype: 'audio/mp4' }))
