			...
			this._events['CB:action,add:relay,message'].unshift(async function(json) {
				try {
					...
					if (m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
						...
						let WAMSG = proto.WebMessageInfo
...
exports.smsg = (xcaa, m, hasParent) => {
	...
	if (m.key) {
		...
		m.sender = m.fromMe ? xcaa.user.jid : m.participant ? m.participant : m.key.participant ? m.key.participant : m.chat
    }
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.fromMe = m.quoted.sender === (xcaa.user && xcaa.user.jid)
			m.quoted.text = m.quoted.text || m.quoted.caption || ''
			m.quoted.mentionedJid = m.quoted.contextInfo ? m.quoted.contextInfo.mentionedJid : []
			m.getQuotedObj = m.getQuotedMessage = async () => {
				if (!m.quoted.id) return false
				let q = await hasParent.loadMessage(m.chat, m.quoted.id, xcaa)
				return exports.smsg(xcaa, q, hasParent)
			}
			...
			if (m.quoted.url) m.quoted.download = (type = 'buffer') => xcaa.downloadM(vM, type)
			
			m.quoted.reply = (text, chatId, options) => xcaa.reply(chatId ? chatId : m.chat, text, vM, options)
			
			m.quoted.copy = () => exports.smsg(xcaa, M.fromObject(M.toObject(vM)))
			
			m.quoted.forward = (jid, forceForward = false) => xcaa.forwardMessage(jid, vM, forceForward)
			
			m.quoted.copyNForward = (jid, forceForward = false, options = {}) => xcaa.copyNForward(jid, vM, forceForward, options)
			
			m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => xcaa.cMod(jid, vM, text, sender, options)
			
			m.quoted.delete = () => xcaa.deleteMessage(m.quoted.chat, vM.key)
		}
		if (m.msg.url) m.download = (type = 'buffer') => xcaa.downloadM(m, type)
		...
		m.reply = (text, chatId, options) => xcaa.reply(chatId ? chatId : m.chat, text, m, { detectLinks: false, thumbnail: global.thumb, ...options })
		
		m.copy = () => exports.smsg(xcaa, M.fromObject(M.toObject(m)))
		
		m.forward = (jid = m.chat, forceForward = false) => xcaa.forwardMessage(jid, m, forceForward)
		
		m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => xcaa.copyNForward(jid, m, forceForward, options)
		
		m.cMod = (jid, text = '', sender = m.sender, options = {}) => xcaa.cMod(jid, m, text, sender, options)
	}
	return m
}
